...
class ZipTeamPassword:
    ...
    def get_groups(self):
        instances = self.contest.instances.select_related(
            "team__institution__country", "user__profile__institution__country"
        ).all()

        for instance in instances:
            if not (instance.group and instance.team and instance.team.name):
                continue
            profile = instance.team.profiles.first()
            if not profile.user:
                continue
            p_instance = PInstance()
            p_instance.user = profile.user.username
            p_instance.password = generate_secret_password(profile.user.id)
            p_instance.team = instance.team.name
            p_instance.institution = (
                instance.institution.name
                if instance.institution and instance.institution.name
                else "Unknown Institution"
            )
            p_instance.group = instance.group
            p_instance.country = (
                instance.institution.country
                if instance.institution and instance.institution.country
        ...
        teams = sorted(
            self.teams,
            key=lambda x: (
                x.institution == "Unknown Institution",
                x.institution,
                x.user,
            ),
        )
    ...
    def generate_zip_team_password(self) -> bytes:
        ...
        with zipfile.ZipFile(content, "w") as zipObj:
            zipObj.writestr(
                str("passwords_%s/allsites.txt" % self.contest_name),
                self.all_password_output,
            )
            zipObj.writestr(
                str("passwords_%s/allteams.txt" % self.contest_name),
                self.not_formated_password_output,
            )
            for site_password in self.site_password_output:
                zipObj.writestr(
                    str(
                        "passwords_%s/sites/%s.txt" % (self.contest_name, site_password)
                    ),
                    self.site_password_output[site_password],
                )
            for institution_password in self.institution_password_output:
                zipObj.writestr(
                    str(
                        "passwords_%s/institutions/%s.txt"
                        % (self.contest_name, institution_password)
                    ),
                    self.institution_password_output[institution_password],
                )
            ...
            for team in teams:
                if team.institution != current_institution:
                    if institution_password_output:
                        ...
                        site_password_output = (
                            site_password_output + institution_password_output
                        )
                ...
                institution_password_output = institution_password_output + str(
                    "user: %s  ||  password: %s  ||  team: %s  ||  institution: %s\n"
                    % (team.user, team.password, team.team, team.institution)
                )
                institution_password_output = (
                    institution_password_output + str("-" * 100) + str("\n")
                )
    ...
    def generate_not_format_passwords(self):
        ...
        for team in teams:
            not_formated_password_output = not_formated_password_output + str(
                "%s||%s||%s||%s||%s||%s\n"
                % (
                    team.user,
                    team.password,
