...
from mog.tasks import report_feedback, report_feedback_to_user, report_feedback_to_assigned
...
def feedback_list(request):
    ...
    return render(request, 'mog/feedback/index.html', {
        'feedbacks': feedbacks,
        'query': query,
...
def filter_feedbacks(user_who_request, subject=None, status=None):
    if user_is_admin(user_who_request):
        ...
        queryset = UserFeedback.objects.filter(Q(sender=user_who_request) | Q(assigned=user_who_request))
...
class FeedbackView(View):
    ...
    def get(self, request, feedback_id, *args, **kwargs):
        ...
        if not user_is_admin(request.user) and feedback.sender != request.user and feedback.assigned != request.user:
            return HttpResponseForbidden()

        status_choices = UserFeedback.STATUS_CHOICES

        admins = User.objects.filter(Q(profile__role='admin') | Q(profile__role='judge')).order_by('profile__role')

        return render(request, 'mog/feedback/feedback.html', {
            'feedback': feedback,
            'status_choices': status_choices,
            'admins': admins
        })
