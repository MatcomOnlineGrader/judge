...
from mog.gating import (
    grant_role_to_user_in_contest,
    revoke_role_to_user_in_contest,
    user_can_bypass_frozen_in_contest,
    user_is_admin,
)
...
def contests(request):
    running, coming, past = Contest.get_all_contests(request.user)
    return render(
        request,
        "mog/contest/index.html",
        {
...
def contest_clarifications(request, contest_id):
    ...
    if user_is_admin(request.user) and contest.needs_unfreeze and contest.is_past:
        msg = _(
            "This contest is still frozen. Go to <b>Actions -> Unfreeze contest </b> to see the final results!"
        )
...
def contest_problems(request, contest_id):
    contest, user = get_object_or_404(Contest, pk=contest_id), request.user
        ...
        if zip_baylor and prefix_baylor:
            try:
                with zipfile.ZipFile(zip_baylor, "r") as zip_ref:
                    process_baylor_file = ProcessImportBaylor(
                        zip_ref, contest_id, prefix_baylor, select_pending_teams_baylor
                    )
                    results = process_baylor_file.handle()
                    for result in results:
                        if result["type"] == "success":
                            messages.success(
                                request, result["message"], extra_tags="success secure"
                            )
                        else:
                            messages.warning(
                                request, result["message"], extra_tags="warning secure"
                            )
...
def contest_manage_export_password(request, contest_id):
    ...
    response["Content-Disposition"] = 'attachment; filename="passwords_{0}.zip"'.format(
        contest.name
    )
    return response
...
def contest_registration(request, contest_id):
    ...
    return render(
        request,
        "mog/contest/registration.html",
        {
            "contest": contest,
            "instances": contest.instances.order_by(
                Lower("group"), "team__institution__name", "team__name"
            ),
        },
    )
...
def contest_rating_changes(request, contest_id):
    ...
    return render(
        request,
        "mog/contest/rating_changes.html",
        {
            "contest": contest,
...
def team_submissions(request, contest_id, team_id):
    ...
    return redirect(
        reverse("mog:contest_submissions", args=(contest_id,))
        + "?username="
...
def remove_contest(request, contest_id):
    ...
    contest = get_object_or_404(Contest, pk=contest_id)
    contest.delete()
    msg = 'Contest "{0}" removed successfully!'.format(contest.name)
...
def register_instance(request, contest, user, team):
    ...
    if not user_is_admin(request.user):
        ...
        if contest.closed and not contest.is_past:
            ...
            msg = _('Registration is closed for contest "{0}"'.format(contest.name))
    ...
    if user and user not in users:
        messages.warning(
            request, _("Invalid user/team combination"), extra_tags="warning"
        )
        return redirect(nxt or reverse("mog:contests"))
    ...
    if team and not contest.allow_teams:
        messages.warning(
            request, _("The contest doesn't allow teams"), extra_tags="warning"
        )
        return redirect(nxt or reverse("mog:contests"))
    ...
    if all(contest.can_register_for_real(u, bypass_closed) for u in users):
        ...
        msg = _(
            "Registration cannot be accomplished, some of the team members cannot participate"
        )
        messages.error(request, msg, extra_tags="danger")
        return redirect(nxt or reverse("mog:contests"))
    ...
    try:
        ...
        for user in users:
            if ContestInstance.objects.filter(contest=contest, user=user).first():
                messages.warning(
                    request,
                    _("User '%s' is already registerd" % user.username),
                    extra_tags="warning",
                )
                continue

            if ContestInstance.objects.filter(
                Q(contest_id=contest.id), Q(team__profiles__user_id=user.id)
            ):
                messages.warning(
                    request,
                    _('The user "%s" is already registered as a team!' % user.username),
                    extra_tags="warning",
                )
                continue

            ContestInstance.objects.create(
                contest=contest, user=user, real=True, group=group or contest.group
            )
            count += 1
...
def contest_registration_multiple_register_team(request, contest_id):
    ...
    try:
        ...
        for team in teams:
            ...
            if ContestInstance.objects.filter(
                contest=contest, team__name=team.name
            ).exists():
                messages.warning(
                    request,
                    _(
                        'There is a team registered with the same name "%s", please check it out, register skiped!'
                        % team.name
                    ),
                    extra_tags="warning",
                )
            ...
            if ContestInstance.objects.filter(contest=contest, team=team).first():
                messages.warning(
                    request,
                    _("Team '%s' is already registerd" % team.name),
                    extra_tags="warning",
                )
                continue
            ...
            if ContestInstance.objects.filter(
                ...
                Q(user_id__in=profiles) | Q(team__profiles__user_id__in=profiles),
            ):
                messages.warning(
                    request,
                    _(
                        'There are an user within the team "%s" that is already registered with another team or as an user, please check it out, register skiped!'
                        % team.name
                    ),
                    extra_tags="warning",
                )
            ...
            ContestInstance.objects.create(
                contest=contest, team=team, real=True, group=group or contest.group
            )
            count += 1
...
def remove_instance(request, instance):
    ...
    if instance.submissions.count() > 0:
        msg = _(
            "Cannot remove registration because the user/team has actions on the contest."
        )
    ...
    if not user_is_admin(user):
        if instance.user != user and (
            instance.team and user.profile not in instance.team.profiles.all()
        ):
            return HttpResponseForbidden()
...
def contest_registration_multiple_unregister(request, contest_id):
    ...
    nxt = request.POST.get("next") or reverse(
        "mog:contest_registration", args=(contest.pk,)
    )
    ...
    try:
        ...
        for instance in instances:
            ...
            if instance.submissions.count() > 0:
                msg = _(
                    "Cannot remove registration of '%s' because the %s has actions on the contest."
                    % (name, "team" if team else "user")
                )
                messages.warning(request, msg, extra_tags="warning")
                continue
            instance.delete()
        ...
        messages.error(
            request, "Unregister user/team: Invalid data!", extra_tags="danger"
        )

    return redirect(nxt)


def set_contest_registration_multiple_enable(
    request, contest_id, instances_selected, is_active
):
    ...
    try:
        ...
        msg = _(
            "Successfully %s %d users" % ("enable" if is_active else "disable", count)
        )
        messages.success(request, msg, extra_tags="success")

    except (ValueError, TypeError):
        messages.error(
            request, "Enable/disable users: Invalid data!", extra_tags="danger"
        )
...
def contest_registration_multiple_enable(request, contest_id):
    ...
    return set_contest_registration_multiple_enable(
        request, contest_id, instances_selected, True
    )
...
def contest_registration_multiple_disable(request, contest_id):
    ...
    return set_contest_registration_multiple_enable(
        request, contest_id, instances_selected, False
    )
...
class ContestCreateView(View):
    ...
    def get(self, request, *args, **kwargs):
        if not user_is_admin(request.user):
            return HttpResponseForbidden()
        return render(request, "mog/contest/create.html", {"form": ContestForm()})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            return render(request, "mog/contest/create.html", {"form": form})
...
def download_json_saris(request, contest_id):
    ...
    response["Content-Disposition"] = 'attachment; filename="saris_{0}.json"'.format(
        contest.name
    )
    return response
...
def rate_contest(request, contest_id):
    ...
    contest = get_object_or_404(Contest, pk=contest_id)
    next = request.POST.get("next") or reverse(
        "mog:contest_problems", args=(contest.id,)
    )
    ...
    if not force_rate and contest.rated:
        msg = _(
            "This contest have ben rated. If you want rate again, then force the operation."
        )
        messages.info(request, msg, extra_tags="info")
        return redirect(next)
    ...
    if (
        RatingChange.objects.filter(
            contest__rated=True, contest__start_date__gt=contest.start_date
        ).count()
        > 0
    ):
...
def unfreeze_contest(request, contest_id):
    ...
    if contest.is_past:
        updated = Submission.objects.filter(
            Q(problem__contest=contest) & ~Q(status="normal")
        ).update(status="normal")
        contest.needs_unfreeze = False
        contest.save()
        msg = "%s %d %s" % (
            _("Contest unfrozen successfully"),
            updated,
            _("submission(s) affected"),
        )
        messages.success(request, msg, extra_tags="success")
    else:
...
def contest_baylor(request, contest_id):
    ...
    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="standing_{0}.csv"'.format(
        contest.name
    )
    ...
    for instance_result in instance_results:
        ...
        if (
            not instance.team
            or not instance.team.icpcid
            or instance.group == contest.group
        ):
            continue
...
class CreateProblemInContestView(View):
    ...
    def get(self, request, contest_id, *args, **kwargs):
        ...
        return render(
            request,
            "mog/contest/create_problem.html",
...
def contest_instances_info(request, contest_id):
    ...
    return JsonResponse(data={"success": True, "data": instances_data})
...
def contest_permission(request, contest_id):
    ...
    return render(
        request,
        "mog/contest/permission.html",
        {
            "contest": contest,
            "permissions": ContestPermission.objects.filter(
                contest=contest, role__in=["judge", "observer"]
            ).order_by("-granted", "role", "user__username"),
...
def contest_add_permission(request, contest_id):
    ...
    nxt = request.POST.get("next") or reverse(
        "mog:contest_permission", args=(contest.pk,)
    )
    ...
    if not observer and not judge:
        msg = _(
            "You need to assign at least one permission (observer / judge) to the users."
        )
        messages.success(request, msg, extra_tags="warning")
        return redirect(nxt)
    ...
    try:
        ...
        for user in users:
            if judge:
                prev_permission = ContestPermission.objects.filter(
                    contest=contest, user=user, role="judge"
                ).first()
                if prev_permission:
            ...
            if observer:
                prev_permission = ContestPermission.objects.filter(
                    contest=contest, user=user, role="observer"
                ).first()
                if prev_permission:
                    set_granted_to_permission(prev_permission, True)
                else:
...
def contest_permission_export(request, contest_id):
    ...
    for permission in permissions:
        row = [permission.user.username, permission.role, permission.granted]
        writer.writerow(row)
        ...
        if csv_permission:
            try:
                ...
                if ",".join(reader[0]).strip() != CSV_PERMISSION_HEADER:
                    raise Exception(
                        "CSV file header must be %s" % CSV_PERMISSION_HEADER
                    )
                ...
                for line in reader[1:]:
                    ...
                    prev_permission = ContestPermission.objects.filter(
                        contest=contest, user=user, role=role
                    ).first()
                    if prev_permission:
        ...
        with zipfile.ZipFile(content, "w") as zipObj:
            for problem in problems:
                ...
                submissions = Submission.objects.filter(
                    problem_id=problem.id, instance_id__in=instances_id
                )
