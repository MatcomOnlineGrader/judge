...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        affected_rows = contests.update(needs_unfreeze=False) if backfill else \
            contests.count()

        if backfill:
            print('Succesfully set needs_unfreeze=False for {} contests'.format(
                affected_rows,
            ))
        else:
            print('{} contests will be affected after executing:\n{}'.format(
                affected_rows,
