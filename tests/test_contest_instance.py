...
class ContestInstanceTestCase(FixturedTestCase):
    ...
    def test_has_failed(self):
        for result in [
            ...
            self.newSubmission(
                instance, user, problem=self.problem2, result=result, status="normal"
            )
            self.assertTrue(
                instance.has_failed_problem(self.problem2) == result.penalty
            )
    ...
    def test_has_solved(self):
        ...
        instance = self.newContestInstance(self.running_contest, user)
        self.newSubmission(
            instance, user, problem=self.problem2, result=self.accepted, status="normal"
        )
    ...
    def test_has_failed_frozen(self):
        for result in [
            ...
            instance = self.newContestInstance(self.running_contest, user)
            self.newSubmission(
                instance, user, problem=self.problem2, result=result, status="frozen"
            )
    ...
    def test_has_failed_death(self):
        for result in [
            ...
            instance = self.newContestInstance(self.running_contest, user)
            self.newSubmission(
                instance, user, problem=self.problem2, result=result, status="death"
            )
            self.assertFalse(instance.has_failed_problem(self.problem2))
    ...
    def test_has_solved_frozen(self):
        ...
        instance = self.newContestInstance(self.running_contest, user)
        self.newSubmission(
            instance, user, problem=self.problem2, result=self.accepted, status="frozen"
        )
    ...
    def test_has_solved_death(self):
        ...
        instance = self.newContestInstance(self.running_contest, user)
        self.newSubmission(
            instance, user, problem=self.problem2, result=self.accepted, status="death"
        )
        self.assertFalse(instance.has_solved_problem(self.problem2))
