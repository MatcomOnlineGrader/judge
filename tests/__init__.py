...
class FixturedTestCase(TestCase):
    ...
    def newUser(self, username, **kargs):
        default = {"username": username, "email": username + "@host.com"}
    ...
    def setUp(self):
        JANUARY_ONE_2018 = timezone.datetime(
            year=2018, month=1, day=1, tzinfo=pytz.timezone(settings.TIME_ZONE)
        )
        ...
        self.wcmp, _ = Checker.objects.get_or_create(
            ...
            backend="testlib.h",
            source=open(
                os.path.join(settings.BASE_DIR, "tests/checkers/wcmp.cpp"), "r"
            ).read(),
        )

        # <results>
        self.accepted, _ = Result.objects.get_or_create(
            name="Accepted", color="green", penalty=False
        )

        self.memory_limit_exceeded, _ = Result.objects.get_or_create(
            name="Memory Limit Exceeded", color="red", penalty=True
        )

        self.pending, _ = Result.objects.get_or_create(
            name="Pending", color="orange", penalty=False
        )

        self.runtime_error, _ = Result.objects.get_or_create(
            name="Runtime Error", color="red", penalty=True
        )

        self.time_limit_exceeded, _ = Result.objects.get_or_create(
            name="Time Limit Exceeded", color="red", penalty=True
        )

        self.wrong_answer, _ = Result.objects.get_or_create(
            name="Wrong Answer", color="red", penalty=True
        )

        self.compilation_error, _ = Result.objects.get_or_create(
            name="Compilation Error", color="blue", penalty=False
        )
