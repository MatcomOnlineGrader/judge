            ...
            for month in range(1, 13):
                ...
                for day in range(1, days + 1):
                    ...
                    if get_special_day(date) == 'valentine':
                        msg = 'get_special_day detects date "%s" as Valentine Day but it is not!' % \
                              date.strftime('%Y-%m-%d')
                        self.assertEqual(date, valentine, msg)
                    else:
                        msg = 'get_special_day fails to detect date %s as Valentine Day' % \
                              date.strftime('%Y-%m-%d')
            ...
            for month in range(1, 13):
                ...
                for day in range(1, days + 1):
                    ...
                    if get_special_day(date) == 'halloween':
                        msg = 'get_special_day detects date "%s" as Halloween but it is not!' % \
                              date.strftime('%Y-%m-%d')
                        self.assertEqual(date, halloween, msg)
                    else:
                        msg = 'get_special_day fails to detect date %s as Halloween' % \
                              date.strftime('%Y-%m-%d')
            ...
            for month in range(1, 13):
                ...
                for day in range(1, days + 1):
                    ...
                    if get_special_day(date) == 'thanksgiving':
                        msg = 'get_special_day detects date "%s" as Thanksgiving but it is not!' %\
                              date.strftime('%Y-%m-%d')
                        self.assertEqual(date, thanksgiving, msg)
                    else:
                        msg = 'get_special_day fails to detect date %s as Thanksgiving' % \
                              date.strftime('%Y-%m-%d')
            ...
            for month in range(1, 13):
                ...
                for day in range(1, days + 1):
                    ...
                    if get_special_day(date) == 'christmas':
                        msg = 'get_special_day detects date "%s" as Christmas but it is not!' % \
                              date.strftime('%Y-%m-%d')
                        self.assertTrue(month == 12 and 19 <= day <= 31, msg)
                    else:
                        msg = 'get_special_day fails to detect date "%s" as Christmas!' % \
                              date.strftime('%Y-%m-%d')
