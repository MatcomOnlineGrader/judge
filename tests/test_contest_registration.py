...
class ContestRegistrationTestCase(FixturedTestCase):
    ...
    def _register_team_in_contest(self, team, contest) -> ContestInstance:
        ...
        self.client.post(
            "/contest/register/{}".format(contest.pk),
            data={
                "team": team.pk,
            },
        )
        return ContestInstance.objects.filter(contest=contest, team=team).first()
    ...
    def test_register_user_success(self):
        user, contest = self.newUser(username="leandro"), self.newContest(
            code="contest"
        )
    ...
    def test_register_team_success(self):
        team, contest = self.newTeam(number_of_users=3), self.newContest(code="contest")
    ...
    def test_register_user_that_has_a_team_already_registered_reuse_the_same_instance(
        self,
    ):
        """
    ...
    def test_admin_endpoints_with_regular_user_in_request(self):
        ...
        response = self.client.post(
            "/contest/register/user/{}".format(contest.pk), {"user": user.pk}
        )
        self.assertEqual(response.status_code, 403)
        response = self.client.post(
            "/contest/register/team/{}".format(contest.pk), {"team": team.pk}
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_user_admin_endpoint_with_admin_in_request(self):
        user, contest = self.newUser(username="alice"), self.newContest(code="contest")
        admin = self.newUser(username="administrator")
        self.updateUserProfile(admin, role="admin")
        self.client.login(
            username=admin.username,
    ...
    def test_contest_allow_team_success(self):
        ...
        """
        contest, user, team = (
            self.newContest(code="contest", allow_teams=True),
    ...
    def test_contest_do_not_allow_team_success(self):
        ...
        """
        contest, user, team = (
            self.newContest(code="contest", allow_teams=False),
            self.newUser(username="leandro"),
            self.newTeam(number_of_users=3),
        )
        user_ci = self._register_user_in_contest(user, contest)
    ...
    def test_that_regular_user_cannot_register_their_team_in_a_closed_contest(self):
        team, contest = self.newTeam(number_of_users=3), self.newContest(
            code="closed-contest", closed=True
        )
        ci = self._register_team_in_contest(team, contest)
