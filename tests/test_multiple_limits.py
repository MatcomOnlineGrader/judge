...
class MultipleLimitsTestCase(FixturedTestCase):
    def setUp(self):
        ...
        self.problem2.multiple_limits = (
            r'{"%s": {"Time": 3},'
            r'"%s": {"Memory": 1024}}' % (self.py2.name, self.py3.name)
        )
    ...
    def test_missing_time(self):
        self.assertTrue(
            self.problem2.time_limit_for_compiler(self.py2) != self.problem2.time_limit
        )
        self.assertTrue(
            self.problem2.time_limit_for_compiler(self.py3) == self.problem2.time_limit
        )
        self.assertTrue(
            self.problem2.time_limit_for_compiler(self.cpp) == self.problem2.time_limit
        )

    def test_missing_memory(self):
        self.assertTrue(
            self.problem2.memory_limit_for_compiler(self.py2)
            == self.problem2.memory_limit
        )
        self.assertTrue(
            self.problem2.memory_limit_for_compiler(self.py3)
            != self.problem2.memory_limit
        )
        self.assertTrue(
            self.problem2.memory_limit_for_compiler(self.cpp)
            == self.problem2.memory_limit
        )
    ...
    def test_missing_limits(self):
        ...
        for problem in [self.problem1, self.problem2]:
            for compiler in self.compilers:
                self.assertTrue(
                    problem.memory_limit_for_compiler(compiler) == problem.memory_limit
                )
                self.assertTrue(
                    problem.time_limit_for_compiler(compiler) == problem.time_limit
                )
