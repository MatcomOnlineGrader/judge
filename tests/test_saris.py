...
class SarisTestCase(FixturedTestCase):
    def setUp(self):
        ...
        self.problems = [self.newProblem(title="Problem%d" % i, position=i, contest=self.past_contest)
                         for i in range(self.number_of_problems)]

        self.users = [self.newUser(username="user%d" % i, is_active=True) for i in range(self.number_of_users)]
        self.instances = [self.newContestInstance(self.past_contest, self.users[i]) for i in range(self.number_of_users)]

    def submit_in_contest(self, user_idx, problem_idx, result, minutes):
        self.newSubmission(self.instances[user_idx], self.users[user_idx], minutes=minutes,
                           problem=self.problems[problem_idx], result=result)
    ...
    def test_counts(self):
        ...
        self.assertTrue(len(result["runs"]) == self.number_of_users * self.number_of_problems)
        self.assertTrue(len(result["contestants"]) == self.number_of_users)
        self.assertTrue(result["freezeTimeMinutesFromStart"] == int(self.past_contest.duration.total_seconds()/60) -
